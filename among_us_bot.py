...
async def start(update, context):
    ...
    await update.message.reply_text(f'Игра началась!', reply_markup=markup)
            ...
            if not fl_enter:
                ...
                for i in range(len(active_tasks_array)):
                    ...
                    if len(active_tasks_array) == 0:
                        ...
                        audio_thread = multiprocessing.Process(target=play_audio, args=("crew_wins.mp3",))
                        audio_thread.start()
            ...
            if not fl_enter:
                ...
                if n == 7 and task_number not in failed_tasks:
                    if len(failed_tasks) < len(active_tasks_array) // 2:
                        ...
                        if len(active_tasks_array) == 0:
                            text += '\nВсе задания выполнены!'
                            audio_thread = multiprocessing.Process(target=play_audio,
                                                            args=("crew_wins.mp3",))
                            audio_thread.start()
                            reply_keyboard = [['/start']]
                            markup = ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=False)
...
async def crash(update, context):
    global crash_fl
    global audio_thread_s
    chat_id = update.message.chat_id
    job_removed = remove_job_if_exists(str(chat_id), context)
    context.job_queue.run_once(task, TIMER, chat_id=chat_id, name=str(chat_id), data=TIMER)
    crash_fl = True
    with open('move_control_file.txt', 'w') as file:
        file.write('Привет от импостеров :)\n')
    audio_thread_s = multiprocessing.Process(target=play_audio, args=("sabbotazh.mp3",))
...
async def meeting(update, context):
    ...
    markup = ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=False)
    if not crash_fl:
        audio_thread_a = multiprocessing.Process(target=play_audio, args=("alarm.mp3",))
        audio_thread_a.start()
...
async def task(context):
    ...
    global failed_tasks
    global crash_fl
    audio_thread = multiprocessing.Process(target=play_audio, args=("imposters_win.mp3",))
    audio_thread.start()
    restart_game()
    reply_keyboard = [['/start']]
    markup = ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=False)
...
async def give_file(update, context):
    global crash_fl
    reply_keyboard = [['/help', '/active_tasks'],
                      ['/complete_task', '/give_file'],
                      ['/emergency_meeting'],
    ...
    markup = ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=False)
    if not crash_fl:
        with open('move_control_file.txt', 'r', encoding="cp1251") as file:
            text = ''
            for el in file.readlines():
                text += el
            if text == '':
                text = 'Посещений не было'
            await update.message.reply_text(text, reply_markup=markup)
    if crash_fl:
        await update.message.reply_text('Сбои в системе. Команда не принята')
...
def main():
    ...
    application.add_handler(CommandHandler('give_file', give_file))
    application.add_handler(CommandHandler('finish', finish))
    text_handler = MessageHandler(filters.TEXT, enter_task)
